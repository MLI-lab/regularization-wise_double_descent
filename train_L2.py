...
def main_worker(gpu, ngpus_per_node, args):
    ...
    param_setup = []
    if args.scale_lr or args.scale_wd:
        if not args.opt.lower() == 'sgd':
            raise ValueError('SGD must be selected when learning rates are scaled!')
        
        for cur_name, cur_lay in model.named_children():
            cur_dict = {'params': cur_lay.parameters()}
            if cur_name in args.scale_lr:
                cur_dict.update({'lr': args.scale_lr[cur_name]})
            if cur_name in args.scale_wd:
                cur_dict.update({'weight_decay': args.scale_wd[cur_name]})
            param_setup.append(cur_dict)

        cur_dict = None

        if args.scale_lr:
            args.initial_lr = [{'lr': args.scale_lr[cur_name]} 
                            if (cur_name in args.scale_lr)
                            else {'lr': args.lr}
                            for cur_name, cur_lay in model.named_children()]
    else:
        param_setup = model.parameters()

    # reformat initial lr decay for training
    if args.initial_lr_decay:
        args.initial_lr_decay = [{'decay': args.initial_lr_decay[cur_name]} 
                                if (cur_name in args.initial_lr_decay)
                                else {'decay': args.inverse_rate}
                                for cur_name, cur_lay in model.named_children()]
    ...
    if args.opt.lower() == 'sgd':
        ...
        optimizer = torch.optim.Adam(param_setup, args.lr,
                                    weight_decay=args.weight_decay)
    elif args.opt.lower() == 'lbfgs':
        print('[INFO] Using LBFGS optimizer instead of SGD.')
        optimizer = torch.optim.LBFGS(param_setup, args.lr,
                                      history_size=20
                                     )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interpolate-only', action='store_true', default=False,
                        help='Train only until slightly after interpolation')
    ...
    parser.add_argument('--layer-file', type=str, default='./model_layers.json', metavar='FILE',
                        help='File containing architecture layer names.')
    ...
    parser.add_argument('--scale-lr', type=str, default='', nargs='*', metavar='SCALE',
                        help='Scale the learning rate of selected layers.')
    parser.add_argument('--scale-wd', type=str, default='', nargs='*', metavar='SCALE',
                        help='Scale the weight decay of selected layers.')
    parser.add_argument('--initial-lr')
    parser.add_argument('--scale-weights')
    parser.add_argument('--initial-lr-decay', type=str, default='', nargs='*', metavar='DECAY',
                        help='Initial learning rate decay of selected layers')
    ...
    if args.scale_lr and (not isinstance(args.scale_lr, dict)):
        with open(args.layer_file) as fn:
            print("Reading model layer names...")
            train_layers = json.load(fn)[args.model]['train_layers']
            print("DONE!")

        args.scale_lr = parse_scaled_lr(args.scale_lr,
                                        train_layers,
                                        default_lr=args.lr)

    if args.scale_wd and (not isinstance(args.scale_wd, dict)):
        with open(args.layer_file) as fn:
            print("Reading model layer names...")
            train_layers = json.load(fn)[args.model]['train_layers']
            print("DONE!")

        args.scale_wd = parse_scaled_lr(args.scale_wd, 
                                        train_layers, 
                                        default_lr=args.weight_decay)
    
    if args.initial_lr_decay and (not isinstance(args.initial_lr_decay, dict)):
        with open(args.layer_file) as fn:
            print("Reading model layer names...")
            train_layers = json.load(fn)[args.model]['train_layers']
            print("DONE!")

        args.initial_lr_decay = parse_scaled_lr(args.initial_lr_decay,
                                        train_layers,
                                        default_lr=args.inverse_rate)

    # rescale weights
    args.scale_weights = False
